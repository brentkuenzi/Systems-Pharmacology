...
def get_info(uniprot_accession_in): #get aa lengths and gene name
	...
	if lines != []:
		...
		if 'GN=' not in header:
			...
			error.close
			return genename
def FilterMaxquant(pSTY_file, PEP_cutoff, MZ_error_cutoff): # Need to finish once we have final format
	...
	samples = []
	for i in data[0]:
		if i.startswith("Reporter intensity "):
			if "not corrected" not in i:
				if "count" not in i:
					samples.append(i)
		...
		filtered = [["Source", "ID","ID2","Accession","Gene","PEP","Mass Error [ppm]"]]
	for i in samples:
		filtered[0].append(i)
	for i in data[1:]:
		if i[Protein_index].startswith("sp"):
			protein = i[Protein_index].split("|")[1]
			temp = ["TMT"]
		#print i[PEP_index]
			if float(i[PEP_index]) < float(PEP_cutoff):
				if abs(float(i[MZerror_index])) < float(MZ_error_cutoff):
					if float(i[Int_index]) != 0:
						GN = get_info(protein)
						ID = protein + "_" + i[modSeq_index] + "_" + i[aa_index] + i[Position_index]
						ID2 = GN + "_" + i[modSeq_index] + "_" + i[aa_index] + i[Position_index]
						temp.append(ID);temp.append(ID2);temp.append(protein); temp.append(GN)
						temp.append(i[PEP_index]);temp.append(i[MZerror_index])
						for j in samples:
			   				temp.append(i[data[0].index(j)])
						filtered.append(temp)
		elif "|" in i[Protein_index]:
			protein = i[Protein_index].split("|")[0]
			protein = protein.replace("sp",i[Protein_index].split("|")[1])
			temp = ["TMT"]
			if float(i[PEP_index]) < float(PEP_cutoff):
				if abs(float(i[MZerror_index])) < float(MZ_error_cutoff):
					if float(i[Int_index]) != 0:
						GN = get_info(protein)
						ID = protein + "_" + i[modSeq_index] + "_" + i[aa_index] + i[Position_index]
						ID2 = "NA" + "_" + i[modSeq_index] + "_" + i[aa_index] + i[Position_index]
						temp.append(ID);temp.append(ID2);temp.append(protein); temp.append("NA")
						temp.append(i[PEP_index]);temp.append(i[MZerror_index])
						for j in samples:
			   				temp.append(i[data[0].index(j)])
						filtered.append(temp)
		else:
			protein = i[Protein_index]
			temp = ["TMT"]
			if float(i[PEP_index]) < float(PEP_cutoff):
				if abs(float(i[MZerror_index])) < float(MZ_error_cutoff):
					if float(i[Int_index]) != 0:
...
def writeTable(DF):
	with open("TMT_output_automated.txt",'w') as x:
		for i in DF:
